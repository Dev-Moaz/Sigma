// app/(main)/wishlist/WishlistClient.tsx
"use client";
...
export default function WishlistClient({ initialLaptops, initialHardware }: WishlistClientProps) {
  ...
  // استخدام البيانات المجلوبة حياً ومطابقتها مع معرّفات الـ wishIds
  const wishedProducts = useMemo(() => {
    const laptops = initialLaptops.map(p => ({ ...p, _type: 'laptop' as const }));
    const hardware = initialHardware.map(p => ({ ...p, _type: 'hardware' as const }));
    
    return [...laptops, ...hardware].filter((p) => wishIds.includes(p.id));
  }, [wishIds, initialLaptops, initialHardware]);
