...
export default function ComparePage() {
  ...
  const products = useMemo(() => {
    return compareIds.map(id => {
      ...
      if (laptop) return { ...laptop, _type: 'laptop' };
      
      const hardware = (hardwareData as HardwareProduct[]).find(p => p.id === id);
      if (hardware) return { ...hardware, _type: 'hardware' };
  ...
  // Define spec categories to compare
  const specKeys = [
  ...
  const getSpecValue = (product: any, spec: any) => {
    ...
    if (spec.type === "spec") {
      if (product._type === 'laptop') {
        ...
        // Hardware specs are usually in product.specs object
              ...
              <div key={product.id} className={`flex flex-col border-r last:border-r-0`} style={{ borderColor: t.borderLight }}>
                ...
                {specKeys.map((spec) => (
                  <div key={spec.label} className="h-20 border-b flex flex-col justify-center px-6 group transition-colors hover:bg-white/5" style={{ borderColor: t.borderLight }}>
                    ...
                    <div className="pc-dm text-[13px] font-bold" style={{ color: spec.type === 'price' ? (theme === 'dark' ? '#34d399' : '#059669') : t.text }}>
                      {spec.type === 'rating' ? (
