// app/(main)/shop/ShopPageClient.tsx
"use client";
...
export default function ShopPageClient({ initialLaptops }: ShopPageClientProps) {
  const { t, isDark } = useTheme();
  const router = useRouter();

  // State
  const [allProducts] = useState<Product[]>(initialLaptops);
  const [filteredProducts, setFilteredProducts] = useState<Product[]>(initialLaptops);
  const [currentPage, setCurrentPage] = useState(1);

  // Sync state if initialLaptops props change (safety sync)
  useEffect(() => {
    setFilteredProducts(initialLaptops);
    setCurrentPage(1);
  }, [initialLaptops]);
  ...
  return (
    ...
    >
      ...
      {/* --- BACKGROUND LAYERS (Clean & Sharp Grid Only) --- */}
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 flex-1 flex flex-col">
        ...
        <CinematicScrollReveal delay={0.1} className="text-center mb-10">
          <div className="inline-flex items-center gap-3 mb-4">
            ...
            <h3 className="hf text-[13px] font-extrabold uppercase tracking-[0.2em]" style={{ color: t.text }}>The Arsenal</h3>
          ...
          >
            Discover our complete range of high-performance rigs. Engineered for maximum
            frames, relentless rendering, and uncompromised power.
          </p>
        ...
        {currentProducts.length > 0 ? (
          ...
          >
            ...
            <p className="text-sm font-medium max-w-md" style={{ color: t.textSecondary }}>
              We couldn't find anything matching your current filters. Try adjusting your search query or removing some filters.
            </p>
          </motion.div>
        )}
