# 💻 Project Context: Sigma Hardware & Laptop Store (Full-Stack Edition)
...
Originally built as a frontend showcase, the platform has been fully upgraded to a **Full-Stack production-ready architecture** integrated with **Supabase PostgreSQL**, secure Row-Level Security (RLS), and Next.js Server Actions, supported by an advanced client-side state machine.

---

## 🛠 Tech Stack & Global Paradigms

- **Framework:** Next.js 15+ (App Router with Server-Client Split Architecture).
- **Database & Auth:** Supabase PostgreSQL, Realtime Session Tokens, Cookie-based Auth Bridge, and Row-Level Security (RLS).
...
## 📂 Core Components & Dynamic Architecture Breakdown
...
- **Key Features:**
  - **Dynamic Search Overlay:** A full-screen search interface that asynchronously queries both `Products` and `Hardware` databases at runtime, featuring local JSON as a resilient fallback.
...
- **Role:** The ultimate Single Product Page (PDP). It dynamically renders a highly immersive, cinematic showcase based on the product's specific hardware metadata fetched live from Supabase.
- **Key Features:**
  ...
  - **Dynamic Content Engine:** Uses helper functions (`getCpuContent`, `getRamContent`, etc.) to read the product's `technical_metadata` and automatically generate matching headlines, 3D accents (e.g., Red for AMD, Blue for Intel), and statistics. Includes strict null-safety fallbacks to prevent runtime crashes.
...
- **Role:** Dynamic client template components that receive live-fetched data props from Server wrappers.
- **Key Features:**
  - **Data Pre-Filtering:** Uses `useMemo` to filter products dynamically fetched from the Supabase Database based on the passed `brandName` or `categoryName` prop before passing the data to the `FilterNavbar`.
...
- **Role:** The listing page specifically for PC Hardware components (CPUs, GPUs, RAM, etc.), receiving pre-fetched dynamic DB arrays.
- **Key Features:**
  - **Schema Context:** Specifically typed for `HardwareProduct` and maps data live from Supabase PostgreSQL (distinct from laptops).
...
- **Key Features:**
  - **Database Synchronization:** Reactively matches `wishIds` from `useWishlist` against the live-fetched product array from Supabase, resolving the limitations of static local lookups.
...
- **Key Features:**
  - **Variants:** Accepts a `variant` prop (`responsive` or `fixed`) to automatically adjust spacing depending on its context.
...
- **Role:** A micro-component used to separate layout sections with a futuristic neon touch.
- **Key Features:**
...
- **Key Features:**
  - **`app/(main)/shop/page.tsx`:** The universal catalog rendering all available Laptops dynamically loaded from Supabase.
  - **`app/(main)/wishlist/page.tsx`:** Subscribes to `useWishlist` to display saved items matched with live database properties.
  - **`app/(main)/deals/page.tsx` (Advanced Aggregation):**
    ...
    - **Union Type Architecture:** Successfully aggregates **BOTH** `Product` (Laptops) and `HardwareProduct` schemas into a single, unified state array `Array<Product | HardwareProduct>` dynamically loaded from Supabase.
...
- **Key Features:**
  ...
  - **Data Fetching:** Decodes the URI component and fetches the dynamic database properties on the server side using the secure product actions, ensuring zero layout shift. If no product matches the ID, it gracefully triggers the Next.js `notFound()` boundary.
...
- **Role:** Structural App Router pages (e.g., `/gaming`, `/business`, `/2-in-1`, `/ultrabooks`) that act as specific dynamic entry points.
- **Key Features:**
  - **DRY Architecture:** Instead of duplicating layout and fetching logic, these pages serve as thin Server Component wrappers. They load all products dynamically on the server from Supabase and pass them to the client-side `LaptopCategoryPage` component, keeping the routing tree clean and highly maintainable.
...
- **Key Features:**
  ...
  - **Type-Safe Data Fetching:** Decodes the URI component and fetches the hardware data dynamically from Supabase, ensuring live-data synchronization.
...
- **Key Features:**
  ...
  - **Dynamic Spec Mapping:** Uses a custom resolver to map data dynamically fetched from Supabase, resolving the dynamic fields (e.g., mapping a Laptop's `technical_metadata.cpu` and a Hardware's `specs.socket`) into a consistent "Processor" or "Graphics" row.
