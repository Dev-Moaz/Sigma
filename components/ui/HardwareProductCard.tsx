...
const VisualRayTracing = ({ color }: { color: string }) => (
  <div className="relative w-[300px] h-[300px] preserve-3d scene-perspective flex items-center justify-center" aria-hidden="true">
...
const VisualTensorCores = ({ color }: { color: string }) => (
  <div className="relative w-[280px] h-[280px] grid grid-cols-4 grid-rows-4 gap-3 p-4 mat-pcb-black border border-white/10 rounded-xl preserve-3d" style={{ transform: 'rotateX(30deg) rotateZ(15deg)' }} aria-hidden="true">
...
const VisualDataLanes = ({ color }: { color: string }) => (
  <div className="relative w-[300px] h-[300px] flex flex-col justify-evenly preserve-3d" style={{ perspective: "1000px", transform: 'rotateX(60deg)' }} aria-hidden="true">
    {[...Array(8)].map((_, i) => (
...
function Counter({ target, suffix = "" }: { target: string | number, suffix?: string }) {
  ...
  return <span ref={ref} aria-live="polite">{val}{suffix}</span>;
}
...
const LiquidButton = ({ children, onClick, active, color, ariaLabel }: LiquidButtonProps) => {
  const { t } = useTheme();
  return (
    ...
    <main className="relative min-h-screen overflow-hidden pb-32 transition-colors duration-500" style={{ backgroundColor: t.bg, color: t.text }}>
      ...
      <section className="relative min-h-[90svh] flex flex-col justify-center pt-24 pb-12 z-10 max-w-[1600px] mx-auto px-6 lg:px-12"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-4">
          <div className="w-3 h-3 rounded-sm animate-pulse" style={{ background: brandColor, boxShadow: `0 0 15px ${brandColor}` }} aria-hidden="true" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col z-20">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="flex flex-col sm:flex-row items-start sm:items-center gap-8">
              ...
              <div className="flex gap-4 w-full sm:w-auto">
                <LiquidButton onClick={handleAddToCart} active={isAdded} color={brandColor} ariaLabel="Add to Cart">
          ...
          <div className="relative h-[400px] lg:h-[600px] flex items-center justify-center z-10 perspective-[2000px]">
            <motion.div style={{ rotateX, rotateY }} animate={{ y: [-15, 15, -15] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="relative gpu-layer w-full h-full drop-shadow-2xl">
              <Image 
                src={primaryImage} 
                alt={product.name} 
                fill 
      ...
      {/* ================= DATA STREAM MARQUEE ================= */}
      <div className="relative z-20 py-6 backdrop-blur-xl overflow-hidden flex whitespace-nowrap border-y mt-12" style={{ backgroundColor: t.cardBg, borderColor: t.borderLight }} aria-hidden="true">
            ...
            <section key={idx} className={`max-w-[1500px] mx-auto px-6 lg:px-12 py-32 flex flex-col ${isReversed ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center justify-between gap-24`}>
              ...
              <div className="w-full lg:w-1/2 flex flex-col">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center border" style={{ borderColor: t.borderLight, backgroundColor: t.cardBg }} aria-hidden="true">
              ...
              <div className="w-full lg:w-1/2 flex justify-center perspective-[2000px]">
                 <motion.div initial={{ rotateY: isReversed ? 40 : -40, opacity: 0, scale: 0.8, x: isReversed ? -100 : 100 }} whileInView={{ rotateY: 0, opacity: 1, scale: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }} className="gpu-layer preserve-3d" aria-hidden="true">
      ...
      <motion.div style={{ y: stickyBarY }} className="fixed bottom-0 left-0 right-0 z-50 p-4 md:p-8 pointer-events-none">
        <div className="max-w-[1200px] mx-auto rounded-2xl backdrop-blur-3xl border shadow-2xl p-4 flex items-center justify-between pointer-events-auto" style={{ backgroundColor: t.dropdownBg, borderColor: t.borderLight }}>
          <div className="hidden md:flex items-center gap-6">
            <div className="relative w-16 h-16 rounded-xl flex items-center justify-center overflow-hidden border p-2" style={{ backgroundColor: t.cardBg, borderColor: t.borderSubtle }}>
              <Image src={primaryImage} alt={product.name} width={48} height={48} className="object-contain" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end">
            ...
            <LiquidButton onClick={handleAddToCart} active={isAdded} color={brandColor} ariaLabel="Quick Buy">
